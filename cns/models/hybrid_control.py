...
# THE control gain, for every consumer. lambda is a free scalar that sets label
# magnitude, and it must be IDENTICAL between label generation and the simulator's
# integration step -- otherwise DAgger rollouts systematically diverge from what the
# labels imply, which is untraceable from any offline metric. Keeping it as a literal
# in several call sites is how the sibling parameter Z came to be 1.0 in the collector
# and d* at deployment for two entire runs (median 10.3 deg of disagreement, max 149).
# Import LAM rather than writing 1.0; `pbvs_straight` is lambda=1 by construction
# (v = -R^T t, w = -u), so changing this alone would DESYNC it -- see assert_lam().
LAM = 1.0


def assert_lam(lam, where=""):
    """Fail loudly if a caller passes a gain that pbvs_straight does not implement.

    The labels come from `pbvs_straight`, which hardcodes lambda=1 in its algebra.
    Any other gain in the control path is therefore a train/deploy mismatch, not a
    tuning choice. If you genuinely want lambda != 1, parameterize pbvs_straight in
    the same commit.
    """
    if abs(float(lam) - LAM) > 1e-12:
        raise ValueError(
            f"control gain {lam} != LAM ({LAM}){' in ' + where if where else ''}; "
            f"labels are generated by pbvs_straight at lambda=1, so this desyncs "
            f"the rollout from the labels (spec Sec. 1.2)")
...
def hybrid_velocity(theta_u, cXg_patch, dXg_patch, R_dc=None, t_dc=None,
    ...
    lam       : control gain
    """
    assert_lam(lam, "hybrid_velocity (Malis form)")
    k = dict(CANONICAL_K)
    if intrinsic:
        k.update(intrinsic)
...
def hybrid_velocity_eq24(t_c, theta_u, cXg_patch, dXg_patch, Z=1.0, lam=LAM,
                         intrinsic=None):
    ...
    """
    assert_lam(lam, "hybrid_velocity_eq24")
    k = dict(CANONICAL_K)
    if intrinsic:
        k.update(intrinsic)
    xc = patch_to_normalized(cXg_patch, **k)
